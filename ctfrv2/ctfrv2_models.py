...
class ConvVarTransformer_Model(tf.keras.Model):
    def __init__(self,
        ...
        if decoder_lags == 'Default':
            self.decoder_lags = int(self.hist_len)
        else:
            self.decoder_lags = int(decoder_lags)
            
        self.num_causal_layers = int(min_power_of_2(self.hist_len)) 
        self.num_layers = num_layers
        self.num_heads = num_heads
        self.kernel_sizes = kernel_sizes
        self.d_model = d_model
    ...
    def call(self, inputs, training):
        ...
        # historical seq encoding
        hist_state_list = []
        for i in range(self.f_len):
            hist_seq = target_copy[:,i:self.hist_len + i,:]
            hist_state = self.hist_encode_layer(hist_seq)
            hist_state_list.append(hist_state)
        hist_state_seq = tf.concat(hist_state_list, axis=1)    # (batch, f_len, d_model)
        decoder_vars_list.append(hist_state_seq)
        future_cols_ordered_list = future_cols_ordered_list + ["{}_state_encoding".format(self.target_col_name)]
